...
def detect_test_framework(project_path: Path) -> dict:
    ...
    backend_manage_py = project_path / "backend" / "manage.py"
    
    if manage_py.exists():
        result["type"] = "django"
        result["framework"] = "manage.py test"
        result["cmd"] = ["python3", "manage.py", "test"]
        result["coverage_cmd"] = ["coverage", "run", "manage.py", "test"]
    elif backend_manage_py.exists():
        result["type"] = "django"
        result["framework"] = "manage.py test"
        # We need to run it from the backend directory or point to it
        result["cmd"] = ["python3", "backend/manage.py", "test"]
        result["coverage_cmd"] = ["coverage", "run", "backend/manage.py", "test"]
...
def run_tests(cmd: list, cwd: Path) -> dict:
    ...
    try:
        ...
        # Try to parse test counts from output (combining stdout and stderr)
        ...
        # Django pattern: "Ran X test in Ys"
        if "django" in str(cmd) or "manage.py" in str(cmd):
            import re
            ...
            if match:
                ...
                if "OK" in output:
                    ...
                    result["tests_failed"] = (int(fail_match.group(1)) if fail_match else 0) + (
